var Scaper = (function($,window,document,undefined) {
	...
	// company, and post count
	var _companyCount, _postCount;
	...
	var render = {
		...
		renderCounts : function(companyCount, postCount){
			_postCount.text(postCount);
			_companyCount.text(companyCount);
		},
			...
			jQuery.ajax({
			    ...
			    success: function (data, textStatus, jqXHR) { 
			    	if(data.status == 'success'){
						...
						render.renderCounts(data.companyResult.count, data.postResult.count);
			    	}
			    }
			});
		},findOnMap : function(){
	...
	var init = {
		initArea : function(objs){
			...
			_postCount = objs.countArea.post;
			_companyCount = objs.countArea.company;
